...
import { cards, installments as initialInstallments } from '@/lib/mockData'
...
export default function CardsPage() {
  ...
  function handleAdd() {
    ...
    const newInst: Installment = {
      ...
      totalAmount: total,
      monthlyAmount: Math.ceil(total / months),
      totalMonths: months,
      paidMonths: 0,
    ...
    setShowModal(false)
    setForm({ cardId: cards[0].id, description: '', totalAmount: '', totalMonths: '6', startDate: new Date().toISOString().slice(0, 10) })
  }
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {cards.map(card => {
          ...
          const monthly = cardInst.reduce((s, i) => s + i.monthlyAmount, 0)
          return (
            <div key={card.id} className="text-white rounded-2xl p-4 shadow-sm" style={{ backgroundColor: card.color }}>
              ...
              <div className="text-xs opacity-80">이달 청구</div>
              <div className="text-base font-bold">{formatKRW(monthly)}</div>
            </div>
          )
        })}
      </div>
      ...
      <div className="space-y-3">
            ...
            <div key={inst.id} className={`bg-white rounded-2xl p-4 shadow-sm ${isDone ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">{card?.name} · 총 {formatKRW(inst.totalAmount)}</div>
                </div>
                <div className="text-right">
                  <div className={`text-sm font-bold ${isDone ? 'text-emerald-600' : 'text-gray-900'}`}>
                    {isDone ? '완료' : `월 ${formatKRW(inst.monthlyAmount)}`}
                  </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-gray-100 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${isDone ? 'bg-emerald-500' : 'bg-blue-500'}`}
                    style={{ width: `${pct}%` }}
                  ></div>
                </div>
                <span className="text-xs text-gray-400 whitespace-nowrap">
                  {inst.paidMonths}/{inst.totalMonths}회차
                  {!isDone && ` (${remaining}회 남음)`}
                </span>
              </div>
            </div>
          )
        ...
        {installments.length === 0 && (
          <div className="text-center py-12 text-gray-400">
            ...
            <div className="text-sm">등록된 할부가 없습니다</div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-5 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold text-gray-900">할부 추가</h2>
            ...
            <div className="space-y-3">
              ...
              <input type="text" placeholder="품목명 (예: 애플 맥북)" value={form.description}
                onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <input type="number" placeholder="총 금액" value={form.totalAmount}
                onChange={e => setForm(f => ({ ...f, totalAmount: e.target.value }))}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <div className="flex gap-2">
                <select value={form.totalMonths} onChange={e => setForm(f => ({ ...f, totalMonths: e.target.value }))}
              ...
              {form.totalAmount && form.totalMonths && (
                <div className="bg-blue-50 rounded-xl p-3 text-sm text-blue-700">
                  월 납입: {formatKRW(Math.ceil(Number(form.totalAmount) / Number(form.totalMonths)))}
                </div>
              )}
