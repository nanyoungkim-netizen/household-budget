...
export default function SavingsPage() {
  ...
  const [form, setForm] = useState({ name:'', bank:'', type:'saving' as 'saving'|'deposit', monthlyAmount:'', interestRate:'', startDate:'', maturityDate:'', currentAmount:'' })
  ...
  function handleAdd() {
    ...
    setSavings([...savings, {
      id:`s${Date.now()}`, name:form.name, bank:form.bank, type:form.type,
      monthlyAmount:Number(form.monthlyAmount)||0, interestRate:rate,
      startDate:form.startDate, maturityDate:form.maturityDate, currentAmount:cur, expectedAmount:cur*(1+rate/100),
    } as Saving])
    setShowModal(false)
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-5 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input type="number" placeholder="연 이율 (%)" value={form.interestRate} onChange={e => setForm(f => ({ ...f, interestRate: e.target.value }))} className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div><label className="text-xs text-gray-400 block mb-0.5">가입일</label><input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                <div><label className="text-xs text-gray-400 block mb-0.5">만기일</label><input type="date" value={form.maturityDate} onChange={e => setForm(f => ({ ...f, maturityDate: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
              </div>
