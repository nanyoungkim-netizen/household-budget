      ...
      {hasData && tab === 'category' && (
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="font-semibold text-sm text-gray-900 mb-4">
            이번 달 카테고리별 지출
            <span className="ml-2 text-gray-400 font-normal">총 {fmtKRW(totalExpense)}</span>
          </div>
          {catExpenses.length > 0 ? (
            <div className="flex flex-col md:flex-row items-center gap-6">
              <ResponsiveContainer width={220} height={220}>
                <PieChart>
                  <Pie data={catExpenses} cx="50%" cy="50%" innerRadius={60} outerRadius={95} dataKey="value" paddingAngle={2}>
                    {catExpenses.map((c, idx) => <Cell key={idx} fill={c.color} />)}
                  </Pie>
                  <Tooltip formatter={(v) => fmtKRW(Number(v))} />
                </PieChart>
              </ResponsiveContainer>
              <div className="flex-1 space-y-2 w-full">
                {catExpenses.map(c => (
                  <div key={c.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 w-10 text-right">{totalExpense > 0 ? (c.value/totalExpense*100).toFixed(1) : 0}%</span>
                  </div>
                ))}
              </div>
            </div>
          ) : (
            <div className="text-center py-8 text-gray-400"><div className="text-sm">이번 달 지출 내역이 없습니다</div></div>
          )}
        </div>
      )}
